...
public:
    ...
    static bool HandleGameObjectAddCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage(LANG_GAMEOBJECT_ADD, objectId, objectInfo->name.c_str(), std::to_string(spawnId).c_str(), x, y, z);
        return true;
    }
    ...
    static bool HandleGameObjectDeleteCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!ownerGuid.IsEmpty())
        {
            ...
            {
                handler->PSendSysMessage(LANG_COMMAND_DELOBJREFERCREATURE, std::to_string(guidLow).c_str(), std::to_string(ownerGuid.GetCounter()).c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }
    ...
    static bool HandleGameObjectTurnCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage(LANG_COMMAND_TURNOBJMESSAGE, std::to_string(object->GetSpawnId()).c_str(), object->GetGOInfo()->name.c_str(), object->GetGUID().ToString().c_str());
